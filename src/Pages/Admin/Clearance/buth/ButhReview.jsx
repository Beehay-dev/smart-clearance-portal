...
const ButhReview = () => {
  ...
  const loadButhClearances = async () => {
    setLoading(true);
    try {
      const result = await getDepartmentClearances('buth');
      ...
      if (result.success) {
        const enrichedClearances = await Promise.all(
          result.clearances.map(async (clearance) => {
            ...
            return {
              id: clearance.id,
              name: studentData.fullName || clearance.studentName || 'Unknown Student',
              matricNumber: studentData.matricNumber || clearance.matricNumber || 'N/A',
              department: studentData.department || clearance.studentDepartment || 'N/A',
              email: studentData.email || 'N/A',
              buthPhoneNumber: clearance.buthPhoneNumber || studentData.buthPhoneNumber || 'N/A',
  ...
  const submitReview = async () => {
    ...
    try {
      // Updates the clearance doc and the student's profile status
      // atomically (see updateClearanceStatus)
      const updateResult = await updateClearanceStatus(
        selectedStudent.id,
        reviewAction,
        adminComment,
        userData?.fullName || 'BUTH Admin'
      );
      ...
      await createNotification(selectedStudent.studentId, {
        title: `BUTH Clearance ${reviewAction === 'approved' ? 'Approved' : 'Rejected'}`,
        message: reviewAction === 'approved' 
          ? 'Your BUTH medical clearance has been approved!' 
          : `Your BUTH clearance was rejected. Reason: ${adminComment}`,
        type: reviewAction === 'approved' ? 'success' : 'error'
      });

      toast.success(`BUTH clearance ${reviewAction} for ${selectedStudent.name}`);
      
      setShowReviewModal(false);
      setAdminComment('');
      setSelectedStudent(null);
      
      await loadButhClearances();
    } catch (error) {
    ...
    return (
      <div className="review-page">
        <div className="review-page-header">
          <div>
            <h1><FaHeartbeat /> BUTH Medical Clearance Review</h1>
            <p>Review and approve student medical clearances</p>
          </div>
        </div>
      ...
      <div className="students-review-list">
        {filteredStudents.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              {filterStatus === 'all' 
                ? 'No BUTH clearance submissions yet' 
              ...
              <div className="student-review-body">
                <div className="medical-details">
                  <div className="detail-item">
                    <span className="detail-label">BUTH Contact:</span>
                    <span className="detail-value">{student.buthPhoneNumber}</span>
                  </div>
                  <div className="detail-item">
                  ...
                  <div className="detail-item">
                    ...
                    <span className="detail-value">{student.documents.length} uploaded</span>
                  </div>
                </div>
                ...
                {student.lastComment && (
                  <div className={`last-comment comment-${student.status}`}>
                    <FaComments />
                    <div>
                      <strong>Medical Feedback:</strong> {student.lastComment}
                      {student.reviewedBy && (
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>
                {reviewAction === 'approved' ? (
                  <>
                    <FaCheckCircle /> Approve BUTH Clearance
                  </>
                ) : (
                  <>
                    <FaTimesCircle /> Reject BUTH Clearance
                  </>
                )}
              </h3>
            ...
            <div className="modal-body">
              <div className="modal-student-info">
                ...
                <p><strong>BUTH Contact:</strong> {selectedStudent.buthPhoneNumber}</p>
              ...
              {reviewAction === 'approved' && (
                <div className="approval-notice">
                  <FaCheckCircle />
                  <p>This will approve the BUTH medical clearance and notify the student.</p>
                </div>
              )}
