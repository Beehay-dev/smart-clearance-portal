...
const HodReview = () => {
  ...
  const loadHodClearances = async () => {
    setLoading(true);

    try {
      const result = await getDepartmentClearances('hod');
        ...
        const enrichedClearances = await Promise.all(
          result.clearances.map(async (clearance) => {
            ...
            return {
              ...
              level: clearance.level || studentData.level || '400 Level',
              cgpa: clearance.cgpa || studentData.cgpa || '0.00',
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      await createNotification(selectedStudent.studentId, {
        title: `HOD Clearance ${reviewAction === 'approved' ? 'Approved' : 'Rejected'}`,
        message: reviewAction === 'approved' 
          ? 'Your HOD clearance has been approved!' 
          : `Your HOD clearance was rejected. Reason: ${adminComment}`,
        type: reviewAction === 'approved' ? 'success' : 'error'
      });

      toast.success(`HOD clearance ${reviewAction} for ${selectedStudent.name}`);
      
      setShowReviewModal(false);
      setAdminComment('');
      setSelectedStudent(null);
      
      await loadHodClearances();
    } catch (error) {
    ...
    return (
      <div className="review-page">
        <div className="review-page-header">
          <div>
            <h1><FaUserTie /> HOD Clearance Review</h1>
            <p>Review and approve student academic clearances</p>
          </div>
        </div>
      ...
      <div className="students-review-list">
        {filteredStudents.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              {filterStatus === 'all' 
                ? 'No HOD clearance submissions yet' 
              ...
              <div className="student-review-body">
                ...
                {student.lastComment && (
                  <div className={`last-comment comment-${student.status}`}>
                    <FaComments />
                    <div>
                      <strong>HOD Feedback:</strong> {student.lastComment}
                      {student.reviewedBy && (
            ...
            <div className="modal-header">
              <h3>
                {reviewAction === 'approved' ? (
                  <>
                    <FaCheckCircle /> Approve HOD Clearance
                  </>
                ) : (
                  <>
                    <FaTimesCircle /> Reject HOD Clearance
                  </>
                )}
              </h3>
            ...
            <div className="modal-body">
              ...
              {reviewAction === 'approved' && (
                <div className="approval-notice">
                  <FaCheckCircle />
                  <p>This will approve the HOD clearance and notify the student.</p>
                </div>
              )}
