...
const SecurityReview = () => {
  ...
  useEffect(() => {
    loadSecurityClearances();
  }, []);

  const loadSecurityClearances = async () => {
    setLoading(true);
    try {
      const result = await getDepartmentClearances('security');
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      const updateResult = await updateClearanceStatus(
        ...
        reviewAction,
        adminComment,
        userData?.fullName || 'Security Admin'
      );
      ...
      await createNotification(selectedStudent.studentId, {
        title: `Security Clearance ${reviewAction === 'approved' ? 'Approved' : 'Rejected'}`,
        message: reviewAction === 'approved' 
          ? 'Your security clearance has been approved!' 
          : `Your security clearance was rejected. Reason: ${adminComment}`,
        type: reviewAction === 'approved' ? 'success' : 'error'
      });

      toast.success(`Security clearance ${reviewAction} for ${selectedStudent.name}`);
    ...
    return (
      <div className="review-page">
        <div className="review-page-header">
          <div>
            <h1><FaShieldAlt /> Security Clearance Review</h1>
            <p>Review and approve student security clearances</p>
          </div>
        </div>
      ...
      <div className="students-review-list">
        {filteredStudents.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              {filterStatus === 'all' 
                ? 'No security clearance submissions yet' 
              ...
              <div className="student-review-body">
                ...
                {student.lastComment && (
                  <div className={`last-comment comment-${student.status}`}>
                    <FaComments />
                    <div>
                      <strong>Security Feedback:</strong> {student.lastComment}
                      {student.reviewedBy && (
            ...
            <div className="modal-header">
              <h3>
                {reviewAction === 'approved' ? (
                  <>
                    <FaCheckCircle /> Approve Security Clearance
                  </>
                ) : (
                  <>
                    <FaTimesCircle /> Reject Security Clearance
                  </>
                )}
              </h3>
            ...
            <div className="modal-body">
              ...
              {reviewAction === 'approved' && (
                <div className="approval-notice">
                  <FaCheckCircle />
                  <p>This will approve the security clearance and notify the student.</p>
                </div>
              )}
