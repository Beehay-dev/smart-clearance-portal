...
const LibraryReview = () => {
  ...
  useEffect(() => {
    loadLibraryClearances();
  }, []);

  const loadLibraryClearances = async () => {
    setLoading(true);
    try {
      const result = await getDepartmentClearances('library');
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      const updateResult = await updateClearanceStatus(
        ...
        reviewAction,
        adminComment,
        userData?.fullName || 'Library Admin'
      );
      ...
      await createNotification(selectedStudent.studentId, {
        title: `Library Clearance ${reviewAction === 'approved' ? 'Approved' : 'Rejected'}`,
        message: reviewAction === 'approved' 
          ? 'Your library clearance has been approved!' 
          : `Your library clearance was rejected. Reason: ${adminComment}`,
        type: reviewAction === 'approved' ? 'success' : 'error'
      });

      toast.success(`Library clearance ${reviewAction} for ${selectedStudent.name}`);
      
      setShowReviewModal(false);
      setAdminComment('');
      setSelectedStudent(null);
      
      await loadLibraryClearances();
    } catch (error) {
    ...
    return (
      <div className="review-page">
        <div className="review-page-header">
          <div>
            <h1><FaBook /> Library Clearance Review</h1>
            <p>Review and approve student library clearances</p>
          </div>
        </div>
      ...
      <div className="students-review-list">
        {filteredStudents.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              {filterStatus === 'all' 
                ? 'No library clearance submissions yet' 
              ...
              <div className="student-review-body">
                ...
                {student.lastComment && (
                  <div className={`last-comment comment-${student.status}`}>
                    <FaComments />
                    <div>
                      <strong>Library Feedback:</strong> {student.lastComment}
                      {student.reviewedBy && (
            ...
            <div className="modal-header">
              <h3>
                {reviewAction === 'approved' ? (
                  <>
                    <FaCheckCircle /> Approve Library Clearance
                  </>
                ) : (
                  <>
                    <FaTimesCircle /> Reject Library Clearance
                  </>
                )}
              </h3>
            ...
            <div className="modal-body">
              ...
              {reviewAction === 'approved' && (
                <div className="approval-notice">
                  <FaCheckCircle />
                  <p>This will approve the library clearance and notify the student.</p>
                </div>
              )}
